    ...
    <form @submit.prevent="formSubmit" class="register-form">
      ...
      <div class="item">
        <div class="passwordText">
          <input :type="passwordShow" @input="registerPasswordBlur" name="password" placeholder="请输入6到20位新密码" v-model="password" />
      ...
      <div class="item">
        <div class="passwordText">
          <input :type="passwordAgainShow" @input="registerPasswordAgainBlur" name="passwordAgain" placeholder="请再次输入新密码" v-model="passwordAgain" />
          
        </div>
        <div class="error" v-if="passwordAgainDecide === false">请再次输入新密码</div>
      ...
      <!-- 确定 -->
      <button type="primary" @click="register" style="width: 100%; padding: 10px; border: none; background-color: #007bff; color: #fff; border-radius: 5px;">确定</button>
    </form>
  </template>
  ...
  methods: {
    ...
    formSubmit() {
      if (this.passwordDecide === 1 && this.passwordAgainDecide === 1 && this.userDecide === 1) {
        axios({
            method:'get',
            url:'http://localhost:9090/user/userInfo?username='+this.username,
            data:{}
        }).then(result=>{
        //成功的回调
        
        if(result.data!="未找到该用户"){
