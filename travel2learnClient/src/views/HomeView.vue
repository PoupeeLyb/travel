<template>
  <div class="center-content">
    ...
    <el-carousel :interval="4000" type="card" height="550px" class="full-width-carousel">
      <el-carousel-item v-for="item in lists.slice(0, 7)" :key="item.id">
        <router-link :to="{ name: 'scene', params: { id: item.id } }" tag="div">
    ...
    <!-- 剩余项目按原样式展示 -->
    <el-row gutter="20">
      <el-col :span="24" v-for="item in lists.slice(7)" :key="item.id">
        <router-link :to="{ name: 'scene', params: { id: item.id } }" tag="div">
...
export default {
  ...
  methods: {
    fetchImages() {
      axios.get('http://localhost:9090/scenelist')
        .then(response => {
